...
const getPuuid = async (gameName, tagLine) => {
    console.log('lol', gameName, tagLine)
    ...
    const res = await APIEUROPE.get(`/riot/account/v1/accounts/by-riot-id/${encodeURI(gameName)}/${encodeURI(tagLine)}?api_key=${apiKey}`, {api_key: apiKey}).catch(err => err)//.then(response => console.log(response.data) )
